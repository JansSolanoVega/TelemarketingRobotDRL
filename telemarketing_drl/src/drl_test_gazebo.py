...
class DRL_test:
    ...
    def read_laser_data_gazebo(self, data):
        ...
        self.laser_info = data
        #print(self.ranges)

        #min distance to obstacle
        laser_data = self.ranges
        self.min_distanceObstacle = min(laser_data)
        index_min = laser_data.index(self.min_distanceObstacle)
        self.angle_obstacle_from_lidar_centre = (self.laser_info.angle_max - self.laser_info.angle_min) * index_min * 180/ (len(self.ranges)*math.pi) - 120
        #print(min_distanceObstacle)
    ...
    def eval_model(self):
        ...
        while self.distance_to_goal>0.3:
            ...
            if (time.time()-t_anterior)>0.1:
                ...
                obs.append(self.distance_to_goal);obs.append(self.angular_error);obs.append(self.min_distanceObstacle);obs.append(self.angle_obstacle_from_lidar_centre)
                obs = np.reshape(obs, (1, 1, len(self.ranges)+4))
